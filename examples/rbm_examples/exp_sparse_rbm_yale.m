...
conf.eNum      = 1000;   % Number of epoch
conf.bNum      = 1;    % Batch number, 0 means it will be decided by the number of training samples
conf.sNum      = 135;  % Number of samples in one batch
...
conf.vis       = 1;
%% Training RBMs
conf.trn_dat_file = strcat(DAT_DIR,'yale_train_data_9.mat');
EXP_DIR_ = strcat(EXP_DIR,conf.sparsity,lm);
...
MX = max(max(imgs));
imgs = (imgs-MN)/(MX-MN);
save_images(imgs,conf.hidNum,32,32,iname);
end
end
end
end
